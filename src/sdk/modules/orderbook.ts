...
function setSnapshotInternal(
  snapshot: { bids: [string, string][]; asks: [string, string][] },
  baseDecimals?: number,
  quoteDecimals?: number
): void {
  TpSdkHybridObject.orderbookDataSetSnapshot(
    snapshot.bids,
    snapshot.asks,
    baseDecimals,
    quoteDecimals
...
export const orderbook = {
   ...
   */
  subscribe: (
    callback: (data: OrderBookViewResult) => void,
    options?: {
      aggregation?: string;
      decimals?: {
        base?: number;
    ...
    const { aggregation, decimals, snapshot } = options || {};
